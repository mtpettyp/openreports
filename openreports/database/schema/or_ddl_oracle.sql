...
create table OR_TAGS (TAG_ID number(10,0) not null, TAGGED_OBJECT_ID number(10,0) not null, TAGGED_OBJECT_CLASS varchar2(255 char) not null, TAG_VALUE varchar2(255 char) not null, primary key (TAG_ID))

create table REPORT (REPORT_ID number(10,0) not null, NAME varchar2(255 char) not null unique, DESCRIPTION varchar2(255 char) not null, REPORT_FILE varchar2(255 char) not null, PDF_EXPORT number(1,0) not null, CSV_EXPORT number(1,0) not null, XLS_EXPORT number(1,0) not null, HTML_EXPORT number(1,0) not null, RTF_EXPORT number(1,0) not null, TEXT_EXPORT number(1,0) not null, EXCEL_EXPORT number(1,0) not null, FILL_VIRTUAL number(1,0) not null, HIDDEN_REPORT number(1,0) not null, REPORT_QUERY clob, DATASOURCE_ID number(10,0), CHART_ID number(10,0), EXPORT_OPTION_ID number(10,0), primary key (REPORT_ID))
...
create table REPORT_LOG (LOG_ID number(10,0) not null, START_TIME timestamp, END_TIME timestamp, STATUS varchar2(255 char), MESSAGE clob, REPORT_ID number(10,0), USER_ID number(10,0), ALERT_ID number(10,0), primary key (LOG_ID))

create table REPORT_PARAMETER (PARAMETER_ID number(10,0) not null, NAME varchar2(255 char) not null unique, TYPE varchar2(255 char) not null, CLASSNAME varchar2(255 char) not null, DATA clob, DATASOURCE_ID number(10,0), DESCRIPTION varchar2(255 char), REQUIRED number(1,0), MULTI_SELECT number(1,0), primary key (PARAMETER_ID))
