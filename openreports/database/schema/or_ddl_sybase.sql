...
create table OR_TAGS (TAG_ID int identity not null, TAGGED_OBJECT_ID int not null, TAGGED_OBJECT_CLASS varchar(255) not null, TAG_VALUE varchar(255) not null, primary key (TAG_ID))
...
create table REPORT_PARAMETER (PARAMETER_ID int identity not null, NAME varchar(255) not null unique, TYPE varchar(255) not null, CLASSNAME varchar(255) not null, DATA text null, DATASOURCE_ID int null, DESCRIPTION varchar(255) null, REQUIRED tinyint null, MULTI_SELECT tinyint null, primary key (PARAMETER_ID), unique (NAME))
