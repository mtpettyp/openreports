...
create table REPORT_LOG (LOG_ID integer not null auto_increment, START_TIME datetime, END_TIME datetime, STATUS varchar(255), MESSAGE text, REPORT_ID integer, USER_ID integer, ALERT_ID integer, primary key (LOG_ID))

create table REPORT_PARAMETER (PARAMETER_ID integer not null auto_increment, NAME varchar(255) not null unique, TYPE varchar(255) not null, CLASSNAME varchar(255) not null, DATA text, DATASOURCE_ID integer, DESCRIPTION varchar(255), REQUIRED bit, MULTI_SELECT bit, primary key (PARAMETER_ID), unique (NAME))
