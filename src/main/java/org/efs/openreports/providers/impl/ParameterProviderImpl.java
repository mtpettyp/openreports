...
public class ParameterProviderImpl implements ParameterProvider {
	protected static Logger log = Logger.getLogger(ParameterProviderImpl.class.getName());

	private DataSourceProvider dataSourceProvider;
	private DateProvider dateProvider;
	private HibernateProvider hibernateProvider;

	public ParameterProviderImpl(DataSourceProvider dataSourceProvider, DateProvider dateProvider,
	...
	public ReportParameterValue[] getParamValues(ReportParameter reportParameter, Map<String, Object> parameters)
		...
		throw new ProviderException(
				reportParameter.getName() + ": param-type " + reportParameter.getType() + " not supported!");
	}

	protected ReportParameterValue[] parseListValues(ReportParameter reportParameter) throws ProviderException {
	...
	protected ReportParameterValue[] getParamValuesFromDataSource(ReportParameter param, Map<String, Object> parameters)
			throws ProviderException {
	...
	protected void validateParameter(ReportParameter parameter, String value) throws ProviderException {
		try {
	...
	protected Object parseParameter(ReportParameter parameter, String value) throws ProviderException {
		try {
	...
	public void loadReportParameterValues(List<ReportParameterMap> reportParameters, Map<String, Object> parameters)
		...
		for (int i = 0; i < reportParameters.size(); i++) {
			...
			try {
				if (rp.getType().equals(ReportParameter.LIST_PARAM) || rp.getType().equals(ReportParameter.QUERY_PARAM)
			...
			if (reportParameter.isMultipleSelect()) {
				...
				if (value != null) {
					try {
						...
						log.warn("Error setting parameter: " + reportParameter.getName() + " : " + e.getMessage());
					}
				}
			}
		}
	...
	public List<ReportParameter> getReportParameters() throws ProviderException {
		String fromClause = "from org.efs.openreports.objects.ReportParameter reportParameter order by reportParameter.name ";

		return (List<ReportParameter>) hibernateProvider.query(fromClause);
	}

	public ReportParameter insertReportParameter(ReportParameter reportParameter) throws ProviderException {
