...
public class ReportScheduleAction extends ActionSupport {
	...
	public String execute() {
		...
		if (!submitScheduledReport) {
			if (scheduleName != null && scheduleName.length() > 0) {
				try {
					...
					startDate = reportSchedule.getStartDate();
		...
		if (startDate == null || startHour == null || startHour.length() < 1 || startMinute == null
				|| startMinute.length() < 1 || startAmPm == null || startAmPm.length() < 1) {
